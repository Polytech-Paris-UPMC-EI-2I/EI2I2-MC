/*---------------------------------------------------------------------------------
*	Fichier	: main.c
*	Description	: Test UART	: Exemple utilisation UART
*				Envoi et reception de caractere (sans interruption)
*				Baud rate : 9600
*
*
...
int main()
	{
	  ...
	  while(!(UCA1IFG & (1 << 1)))
	   ...
	   UCA1TXBUF = Caractere_Recu;     // ecriture du caractere dans le registre de transmission
...
/*----- Commentaires et explications fonctionnement UART--------
	...
	//2. Choix horloge et configuration de la frequence pour le module USCIA0
		// Nous preons SMCLK configurer pour 1MHz Calibre: bits b7b6->"11"
		UCA1CTL1 |= (1<<7);
		UCA1CTL1 |= (1<<6);
	...
	// 4.1 Choix asynchrone : bit b0 de UCAxCTL0 -> '0'
		UCA1CTL0 = UCA1CTL0&(~(1<<0));
	// 4.2 choix mode UART : bits b2b1 de UCA0CTL0 ->"00"
		UCA1CTL0 = UCA1CTL0&(~(1<<2));
		UCA1CTL0 = UCA1CTL0&(~(1<<1));

	// 5.1 Choix nombre de bits STOP: Ici 1 BIT STOP: bit b3(UCSPB) de UCA0CTL0 ->'0'
		UCA1CTL0 = UCA1CTL0&(~(1<<3));
	//	5.2 Choix taille du format de donnees des caracteres: Caractere coder 8bits
		// bit b4 de UCA0CTL0->'0'
		UCA1CTL0 = UCA1CTL0&(~(1<<4));
	// 5.3. Choix bit de parite ou pas : Pas de bit de parite : bit b7(UCPEN) ->'0'
		UCA1CTL0 = UCA1CTL0&(~(1<<7));
	// 5.4. Choix sens de tranfert des bit: LSB en premier: Bitb5 (UCMSB)de UCA0CTL0 ->'0'
		UCA1CTL0 = UCA1CTL0&(~(1<<5));

	// 6. Configurer la vitesse de transfert : 9600
		UCA1BR0 = 0x6D; // ValeurBaudrate=1048576/9600 = 109.22 =109=0x6D
		UCA1BR1 = 0x00;

	// 7. Relacher le Reset (Deactiver) de la machine a etat du USCI_A0
		// bit b0 (UCSWRST) de UCA0CTL1 -> '0'
		UCA1CTL1 = UCA1CTL1&(~(1<<0)); //
